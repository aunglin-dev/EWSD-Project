...
import {
  ...
  meetingNotificationEmailForTutor,
...
meetingSchema.pre("save", async function (next) {
  try {
    ...
    if (existingMeeting) {
      return next(
        new Error(
          "A meeting with the same date, time, student, and tutor already exists."
        )
      );
    }
...
meetingSchema.post("save", async function (doc) {
  try {
    ...
    const allocation = await mongoose
      .model("Allocation")
      .findById(doc.allocationId)
      .populate("tutor")
      .populate("student");
    ...
    if (!tutor || !student) {
      console.error(
        "Tutor or student not found in allocation:",
        doc.allocationId
      );
      return;
    }
    ...
    const { subjectStudent, messageStudent } = meetingNotificationEmail(
      "created",
      tutor,
      student,
      doc
    );
    const { subjectTutor, messageTutor } = meetingNotificationEmailForTutor(
      "created",
      tutor,
      student,
      doc
...
meetingSchema.pre("findOneAndUpdate", async function (next) {
  try {
    ...
    if (existingMeeting) {
      return next(
        new Error(
          "A meeting with the same date, time, type, student, and tutor already exists."
        )
      );
    }
