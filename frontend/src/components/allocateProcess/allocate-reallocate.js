...
export default function AllocateReallocate() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const allocations = { "tutor": tutorResponse.data, "allocations": allocationsResponse.data?.filter(allocation => allocation.tutor._id === id && allocation) };
    ...
    return (
        ...
        > */}
                ...
                {!loading && successMsg && (
                    <span style={{ fontSize: "14px", fontWeight: "500", marginLeft: "30px", padding: "5px 25px", borderRadius: "20px", backgroundColor: "rgba(0, 200, 0, 0.4)", }}>
                        {successMsg}
                    </span>
                )}
            </Box>
                        ...
                        <Box >
                            <Box display="flex" justifyContent="space-between" alignItems="center" marginBottom="30px" width="100%">
                                ...
                                <Typography variant={isSmallestScreens ? "caption" : "subtitle2"} display="flex" alignItems="center" gap="4px">
                                    ...
                                    {allocatedTutorStudents.allocations.length} students assigned
                                </Typography>
                            </Box>

                            <Box display="flex" flexDirection="column" gap="30px">
                                {allocatedTutorStudents?.allocations?.length && allocatedTutorStudents?.allocations?.map((allocation, index) => (
                                    ...
                                    >
                                        <Box flex="2" overflow="hidden">
                                            <Typography fontWeight="500">{allocation.student.name}</Typography>
                                            <Typography variant={isSmallestScreens ? "caption" : "subtitle2"} display="flex" alignItems="center" gap="4px">
