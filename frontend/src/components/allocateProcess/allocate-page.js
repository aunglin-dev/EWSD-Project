...
export default function AllocatePage() {
  ...
  const allocatedTutorStudents = allTutors.map((tutor) => {
    const allocations = allocationListdata.filter(
      (allocation) => allocation.tutor._id === tutor._id && allocation.student
    );
        ...
        {loading ? (
          ...
          <>
            {allocatedTutorStudents
              ?.filter((selected) => selected.tutor._id === selectedTutor._id)
              .map((allocation, index) => (
                ...
                >
                  <Box
                    ...
                    flex="2"
                  >
                    <Typography fontSize={isSmallestScreens && "16px"}>
                    ...
                    {allocation.allocations.length ? (
                      ...
                      >
                        <CircleIcon
                          sx={{
                            display: isSmallestScreens && "none",
                            color: "#009900",
                            width: "18px",
                            height: "18px",
                          }}
                  ...
                  <Box flex="1" display="flex" justifyContent="end">
                    <Button
                      ...
                      onClick={() =>
                        navigate(`/allocateReallocate/${allocation.tutor._id}`)
                      }
                    >
                      {allocation.allocations.length
                        ? "Reallocate"
          ...
          <>
            ...
            >
              <Button
                variant={!assigned && !notAssigned ? "contained" : "outlined"}
              ...
              <Button
                variant={notAssigned ? "contained" : "outlined"}
                sx={{ fontSize: isSmallestScreens ? "12px" : "16px" }}
                onClick={() => {
                  setAssigned(false);
                  setNotAssigned(true);
                ...
                {assigned ? (
                  <>
                    {allocatedTutorStudents.filter(
                      (assigned) => assigned.allocations.length
                    ).length > 0 ? (
                      allocatedTutorStudents
                        .filter((assigned) => assigned.allocations.length)
                        ?.map((allocation, index) => (
                          <Box
                            key={index}
                            display="flex"
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                ...
                                />
                                {allocation.allocations.length} students
                                assigned
                              </Typography>
                            </Box>
                            <Box flex="1" display="flex" justifyContent="end">
                  ...
                  <>
                    {allocatedTutorStudents.filter(
                      (assigned) => assigned.allocations.length < 1
                    ).length > 0 ? (
                      allocatedTutorStudents
                        .filter((assigned) => assigned.allocations.length < 1)
                        ?.map((allocation, index) => (
                          <Box
                            key={index}
                            display="flex"
